...
void doit(int fd)
{
  ...
  Rio_readinitb(&rio, fd); // rio 구조체와 fd(connfd)를 연결 
  Rio_readlineb(&rio, buf, MAXLINE); // 텍스트줄을 읽음 (rio -> buf 복사)

  printf("Request line:\n");
  printf("%s", buf); // 요청라인 나옴
  sscanf(buf, "%s %s %s", method, uri, version);

  sscanf(strstr(uri, "http://"), "http://%s", tmp); // tmp에 원하는 값을 넣음 (여기서는 uri)
                                                    // strstr(string1, string2) : string1에서 string2의 첫번째 표시를 찾음
  
  
  if((p = strchr(tmp, ':')) != NULL){ // strchr : string에서 c라는 문자를 찾아서 포인터 리턴, 찾지못하면 NULL 리턴
    *p = '\0'; // p 값을 '\0'으로 변경
    sscanf(tmp, "%s", dest);  // dest는 호스트 주소 (포트 포함)
    sscanf(p+1, "%s", tmp2);  // temp2는 포트 번호
    // printf("%s\n", dest);
    // printf("%s\n", tmp2);

    q = strchr(tmp2, '/');    // tmp2에서 '/'에 해당하는 값을 찾음
    *q = '\0';                // 해당값 '\0' 처리
    sscanf(tmp2, "%s", port); // tmp2를 포트로 바꿈
    // printf("%s\n", port);
    *q = '/';                 // 
    sscanf(q, "%s", new_uri); // q는 new_uri (뒤에 '/'가 없을 때를 의미?)
    // printf("%s\n", new_uri);
  ...
  send_request(new_uri, port, fd); // new_uri, port를 가지고 fd로 send_request
...
int send_request(char *uri, char *port, int fd){
  ...
  rio_t rio;
  clientfd = Open_clientfd(dest, port); // dest는 전역변수
  // printf("%d\n", clientfd);
  ...
  Rio_writen(clientfd, buf, strlen(buf)); // buf -> clientfd
  // printf("---------------------------------------\n");
  printf("%s", buf);
