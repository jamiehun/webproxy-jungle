...
int main(int argc, char **argv) {
  ...
  while (1) {
    ...
    // 클라이언트로부터의 연결 요청이 듣기식별자 listenfd에 도달 -> 그 후 addr 내의 소켓주소 채우기 -> 클라이언트와 통신하기 위해 사용될 수 있는 connected descriptor 
    // 듣기 소켓을 오픈
...
void doit(int fd)
{
  ...
  Rio_readinitb(&rio, fd); // 읽기 버퍼와 연결
  /* 요청라인을 읽고 분석 */
  Rio_readlineb(&rio, buf, MAXLINE); // 텍스트 줄을 읽음
  ...
  if (is_static) { /* Serve static content */
    ...
    serve_dynamic(fd, filename, cgiargs); /* 정적 컨텐츠를 서버에게 제공 */
